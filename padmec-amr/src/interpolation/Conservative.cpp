...
double max_value(pFace triangle){
	double xyz[3], p0x, p0y, p1x, p1y, p2x, p2y, v1, v2, v3;

	V_coord(F_vertex(triangle, 0),xyz);
	p0x = xyz[0];
	p0y = xyz[1];
	v1 = circle_func(p0x, p0y);

	V_coord(F_vertex(triangle, 1), xyz);
	p1x = xyz[0];
	p1y = xyz[1];
	v2 = circle_func(p1x, p1y);

	V_coord(F_vertex(triangle, 2), xyz);	
	p2x = xyz[0];
	p2y = xyz[1];
	v3 = circle_func(p2x, p2y);

	return max(v1,max(v2,v3));
}


double min_value(pFace triangle){
	double xyz[3], p0x, p0y, p1x, p1y, p2x, p2y, v1, v2, v3;
...
double mesh_intersection(pFace new_face, queue<pFace> overlapped_elements, vector<int> overlapped_IDelements, double* vals){
	...
	while(!overlapped_elements.empty()){
		...
		aux_max = max_value(old_face);
		aux_min = min_value(old_face);
		if(!first){
